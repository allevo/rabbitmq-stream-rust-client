...
use crate::{client2::{Client2, CloseReason, ResponseHandler}, environment2::Environment2, error::ConsumerStoreOffsetError};
...
impl ConsumerBuilder {
    pub async fn build(mut self, stream: &str) -> Result<Consumer, ConsumerCreateError> {
        ...
        let msg_handler = ConsumerMessageHandler(consumer.clone(), stream.to_string());
        let msg_handler = Box::new(msg_handler);
        ...
        if !response.is_ok() {
            return Err(ConsumerCreateError::Create {
...
struct ConsumerMessageHandler(Arc<ConsumerInternal>, String);

#[async_trait::async_trait]
impl ResponseHandler for ConsumerMessageHandler {
    #[instrument(skip(self, close_reason), fields(stream = %self.1))]
    async fn on_closed(&self, close_reason: CloseReason) {
        panic!("Consumer3: on_closed {:?}", close_reason);
    }

    #[instrument(skip(self, response), fields(stream = %self.1))]
    async fn handle_response(&self, response: Response) {
        error!("Consumer3: handle_response {:?}", response);
