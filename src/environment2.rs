...
use crate::{client2::Client2, consumer3::ConsumerBuilder, error::{ConsumerCreateError, ProducerCreateError, StreamDeleteError}, producer2::{NoDedup, ProducerBuilder}, stream_creator2::StreamCreator2, superstream2::RoutingStrategy, superstream_consumer2::SuperStreamConsumerBuilder2, superstream_producer2::SuperStreamProducerBuilder, types::StreamCreator, ClientOptions, MetricsCollector, RabbitMQStreamResult, TlsConfiguration};
...
impl Environment2 {
    ...
    async fn boostrap(options: EnvironmentOptions) -> RabbitMQStreamResult<Self> {
        // check connection
        let client = Client2::connect(options.client_options.clone()).await?;
        client.close().await;
        Ok(Environment2 { options })
    }
    ...
    pub(crate) async fn create_producer_client(
    ...
    ) -> Result<Client2, ProducerCreateError> {
        ...
        if let Some(metadata) = client.metadata(vec![stream.to_string()]).await?.get(stream) {
            ...
            if load_balancer_mode {
                ...
                loop {
                    let temp_client = Client2::connect(options.clone()).await?;
                ...
                client.close().await;
                client = Client2::connect(ClientOptions {
    ...
    pub(crate) async fn create_consumer_client(
        ...
        stream: &str,
        client_provided_name: String,
    ) -> Result<Client2, ConsumerCreateError> {
        let mut opt_with_client_provided_name = self.options.client_options.clone();
        opt_with_client_provided_name.client_provided_name = client_provided_name.clone();
    ...
    pub fn stream_creator(&self) -> StreamCreator2 {
        StreamCreator2::new(self.clone())
    }
    ...
    pub fn super_stream_consumer(&self) -> SuperStreamConsumerBuilder2 {
        SuperStreamConsumerBuilder2 {
    ...
    pub(crate) async fn create_client(&self) -> RabbitMQStreamResult<Client2> {
        Client2::connect(self.options.client_options.clone()).await
    }
    pub(crate) async fn create_client_with_options(
        &self,
        opts: impl Into<ClientOptions>,
    ) -> RabbitMQStreamResult<Client2> {
        Client2::connect(opts).await
    }
...
impl EnvironmentBuilder {
    pub async fn build(self) -> RabbitMQStreamResult<Environment2> {
        Environment2::boostrap(self.0).await
    }
