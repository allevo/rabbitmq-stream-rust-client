...
use fake::{Fake, Faker};
use rabbitmq_stream_client::client2::Client2;
...
impl TestClient2 {
    pub async fn create() -> TestClient2 {
        Self::create_with_option(ClientOptions::default()).await
    }

    pub async fn create_with_option(options: ClientOptions) -> TestClient2 {
        let stream: String = Faker.fake();
        let client = Client2::connect(options).await.unwrap();
    ...
    pub async fn create_super_stream() -> TestClient2 {
...
impl TestEnvironment {
    pub async fn create() -> TestEnvironment {
        let stream: String = Faker.fake();
        let env = Environment2::builder().build().await.unwrap();
