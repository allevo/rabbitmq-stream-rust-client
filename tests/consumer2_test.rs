...
use rabbitmq_stream_client::{
    consumer3::{Consumer, Delivery, FilterConfiguration}, environment2::Environment2, error::{
        ClientError, ConsumerCloseError, ConsumerDeliveryError, ConsumerStoreOffsetError,
        ProducerCloseError,
    }, producer2::{NoDedup, Producer}, superstream2::{HashRoutingMurmurStrategy, RoutingKeyRoutingStrategy, RoutingStrategy}, superstream_consumer2::SuperStreamConsumer2, types::{ByteCapacity, Message, OffsetSpecification, SuperStreamConsumer}
};
...
async fn super_stream_consumer_test() {
    ...
    let mut super_stream_consumer: SuperStreamConsumer2 = env
        .env
...
async fn consumer_close_test() {
    ...
    println!("producer send_with_confirm");
    let _ = producer
        .send_with_confirm(Message::builder().body("message").build())
        .await
        .unwrap();

    println!("producer send_with_confirm done");
    let handle = consumer.handle();
    let delivery = consumer.next().await;

    println!("consumer next done");
...
async fn new_super_stream_single_active_consumer_test_1() {
    let _ = tracing_subscriber::fmt::try_init();

    let super_stream: String = Faker.fake();
    let environment = Environment2::builder().build().await.unwrap();
    ...
    // Create consumer with SAC enabled
    let mut super_stream_consumer: SuperStreamConsumer2 = env
        .env
        .super_stream_consumer()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 10)]
async fn new_super_stream_single_active_consumer_test_2() {
    let _ = tracing_subscriber::fmt::try_init();
    ...
    // Create consumer with SAC enabled
    let stream_consumer_0 = env
        .env
        .consumer()
        .build(&format!("{}-0", env.super_stream))
        .await
        .unwrap();
    let stream_consumer_1 = env
        .env
        .consumer()
        .build(&format!("{}-1", env.super_stream))
        .await
        .unwrap();
    let stream_consumer_2 = env
        .env
        .consumer()
        .build(&format!("{}-2", env.super_stream))
        .await
        .unwrap();
    ...
    let mut a = tokio_stream::StreamExt::merge(
        tokio_stream::StreamExt::merge(stream_consumer_0, stream_consumer_1),
        stream_consumer_2,
    );
    ...
    let messages = messages.into_iter()
        .map(|delivery| {
            String::from_utf8(delivery.message().data().unwrap().to_vec()).unwrap()
        })
        .collect::<Vec<_>>();
    assert_eq!(messages, vec!["message0", "message1", "message2"]);
}

/*
#[tokio::test(flavor = "multi_thread")]
async fn super_stream_single_active_consumer_test_2() {
    let _ = tracing_subscriber::fmt::try_init();
    ...
    // warn!("create super_stream_consumer_2 ");
    let mut super_stream_consumer_2: SuperStreamConsumer2 = env
        .env
        .super_stream_consumer()
        .name("super-stream-with-sac-enabled")
        .enable_single_active_consumer(true)
    ...
    // warn!("create super_stream_consumer_3 ");
    let mut super_stream_consumer_3: SuperStreamConsumer2 = env
        .env
        .super_stream_consumer()
        .name("super-stream-with-sac-enabled")
        .enable_single_active_consumer(true)
