...
use fake::{Fake, Faker};
use rabbitmq_stream_client::client2::{Client2, CloseReason, ResponseHandler};
...
async fn client_connection_test() {
    let client = Client2::connect(ClientOptions::default()).await.unwrap();
...
async fn client_connection_with_properties_test() {
    ...
    let client = Client2::connect(opts).await.unwrap();
    assert_ne!(client.server_properties().await.len(), 0);
    assert_ne!(client.connection_properties().await.len(), 0);
}
...
async fn client_create_stream_test() {
    TestClient2::create().await;
}

#[tokio::test(flavor = "multi_thread")]
async fn client_create_stream_error_test() {
    let test = TestClient2::create().await;
...
async fn client_create_and_delete_super_stream_test() {
    let _test = TestClient2::create_super_stream().await;
}

#[tokio::test(flavor = "multi_thread")]
async fn client_create_super_stream_error_test() {
    let test = TestClient2::create_super_stream().await;
...
async fn client_publish() {
    ...
    struct ClientResponseHandler(Sender<DeliverCommand>);
    ...
    impl ResponseHandler for ClientResponseHandler {
        ...
        async fn handle_response(&self, response: Response) {
            if let ResponseKind::Deliver(delivery) = response.kind() {
                self.0.send(delivery.clone()).await.unwrap();
            }
        }
    }
    let handler = Box::new(ClientResponseHandler(tx));
    let _ = test
        .client
        .subscribe(
            1,
...
async fn client_drop_connection() {
    ...
    let options = ClientOptions::builder()
        ...
        .heartbeat(2)
        .build();
    let test = TestClient2::create_with_option(options).await;
